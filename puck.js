...
var Puck = (function() {
  ...
  function handleQueue() {
    ...
    if (q.type == "eval")
      puck.eval(q.expr, beaconName, q.cb);
    else if (q.type == "write")
      puck.write(q.data, q.callback, q.callbackNewline);
    else
      log(1, "Unknown queue item " + JSON.stringify(q));
  }
  ...
  function write(data, callback, callbackNewline) {
    ...
    function onWritten() {
      ...
      var maxTime =
          300; // 30 sec - Max time we wait in total, even if getting data
      var dataWaitTime = 3;
      var maxDataTime =
          dataWaitTime; // 300ms - max time we wait after having received data
